...
public class RegistrationHandler implements HandlerStrategy {
  ...
  public void handleRequest(HandlerContext ctx) {
    if (ctx.getUpdate().hasCallbackQuery()) {
      if (BotButtons.BACK_DATA.equals(ctx.getUpdate().getCallbackQuery().getData())) {
        resolveBackButton(ctx.getUpdate(), ctx.getUser());
      }
      ctx.getUser()
          .setLastSentMessageId(MessageUtils.getCorrectMessageIdWithOffset(ctx.getUpdate()));
      switch (ctx.getUser().getUserState()) {
        case AWAITING_INSTITUTE -> getInstitute(ctx.getUpdate(), ctx.getUser());
        case AWAITING_FACULTY -> getFaculty(ctx.getUpdate(), ctx.getUser());
        case AWAITING_COURSE -> getCourse(ctx.getUpdate(), ctx.getUser());
        case AWAITING_GROUP -> getGroup(ctx.getUpdate(), ctx.getUser());
        case AWAITING_REGISTRATION_DATA_CONFIRMATION ->
            confirmRegistration(ctx.getUpdate(), ctx.getUser());
      }
    }
  }
  ...
  private void getInstitute(Update update, User user) {
    ...
    for (Institute institute : instituteList) {
      builder.append(institute.getId()).append(". ").append(institute.getInstituteName())
          .append("\n");
    }
  ...
  private void getFaculty(Update update, User user) {
    ...
    for (Faculty faculty : facultyList) {
      builder.append(faculty.getFacultyId()).append(". ").append(faculty.getFacultyName())
          .append("\n");
    }
    messageSender.sendMessage(
        user,
  ...
  public void confirmRegistration(Update update, User user) {
    ...
    if (BotButtons.YES_DATA.equals(callbackData)) {
      Institute institute = instituteService.getByName(
