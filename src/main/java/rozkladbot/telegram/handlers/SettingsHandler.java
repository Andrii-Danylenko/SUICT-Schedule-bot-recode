...
public class SettingsHandler implements HandlerStrategy {
  ...
  public void handleRequest(HandlerContext ctx) {
    ...
    messageSender.sendMessage(
        ...
        BotMessageConstants.SETTINGS_MENU.formatted(
            ...
            user.isBroadcasted() ? BotMessageConstants.IS_BROADCASTED
                : BotMessageConstants.IS_NOT_BROADCASTED
        ),
        KeyBoardFactory.getSettings(user.isBroadcasted()), ctx.isOverrideMessage(), update);
    user.setLastSentMessageId(MessageUtils.getCorrectMessageIdWithOffset(update));
  }
}
