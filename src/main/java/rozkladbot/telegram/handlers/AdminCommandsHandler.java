...
public class AdminCommandsHandler implements HandlerStrategy {
  ...
  public void handleRequest(HandlerContext ctx) {
    switch (ctx.getUser().getUserState()) {
      case AWAITING_USERS_LIST -> getAllConnectedUsers(ctx.getUpdate(), ctx.getUser());
      case AWAITING_APPLICATION_TERMINATION -> terminateApplication(ctx.getUpdate(), ctx.getUser());
      case AWAITING_DATA_SYNC -> synchronizeLocalData(ctx.getUpdate(), ctx.getUser());
      case AWAITING_MESSAGE_SENDING -> sendBroadcastMessage(ctx.getUpdate());
    }
  }
  ...
  private void terminateApplication(Update update, User user) {
    messageSender.sendMessage(user, BotMessageConstants.APP_TERMINATION_STARTED, null, false,
        update);
    sessionShutdownManager.shutdown();
    messageSender.sendMessage(user, BotMessageConstants.APP_TERMINATION_FINISHED, null, true,
        update);
  }

  private void synchronizeLocalData(Update update, User user) {
    messageSender.sendMessage(user, BotMessageConstants.LOCAL_DATA_SYNC_STARTED, null, false,
        update);
    boolean hasMessage = update.hasMessage();
    localDataSynchronizer.synchronize(hasMessage ? update.getMessage().getText() : "-all");
    messageSender.sendMessage(user, BotMessageConstants.LOCAL_DATA_SYNC_FINISHED, null, true,
        update);
  }
